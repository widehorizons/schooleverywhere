...
class ConferenceStudent extends StatefulWidget {
  final String type;

  const ConferenceStudent(this.type);
  @override
  State<StatefulWidget> createState() {
    return new _ConferenceStudentState();
  }
}

class _ConferenceStudentState extends State<ConferenceStudent> {
  ...
  Future<void> JoinConferenceStatus(String roomChannelName, String staffid, String StaffSubjectId) async{
    EventObject objectEvent = new EventObject();
    objectEvent = await JoinConference(roomChannelName,userId!,userAcademicYear!,StaffSubjectId,userSection!,userstage!,usergrade!,userclass!,usersemester!,typeUser!);
    // print("kkkkkkk" + objectEvent.object);
    Map? data = objectEvent.object as Map?;
    if (objectEvent.success!) {
      JoinStaff = data!['data'];
