...
import 'UnPaidVacationRequest.dart';
import 'VacationRequest.dart';
import 'package:schooleverywhere/config/flavor_config.dart';

class PreviousUnPaidVacationRequest extends StatefulWidget {
  final String id;
  final String year;
  final String section;
  const PreviousUnPaidVacationRequest(this.id, this.year, this.section);

  @override
  State<StatefulWidget> createState() {
    return new _PreviousUnPaidVacationRequestState();
  }
}

class _PreviousUnPaidVacationRequestState
    extends State<PreviousUnPaidVacationRequest> {
  bool isLoading = false;
  bool pressed = false, checkBoxValue = false;
  List<dynamic> unPaidVacationRequests = [];
  final format = DateFormat("yyyy-MM-dd");
  TextEditingController dateFrom = new TextEditingController();
  TextEditingController dateTo = new TextEditingController();
  List<dynamic> dropUnPaidVacationRequest = [];
  initState() {
    super.initState();
  }

  Future<void> syncPreviousUnPaidVacationRequest() async {
    EventObject objectEventSt = await unPaidPreviousVacationRequest(
        widget.id, dateFrom.text, dateTo.text);
    if (objectEventSt.success!) {
      ...
      setState(() {
        unPaidVacationRequests = listOfColumns;
        isLoading = true;
      });
    } else {
  ...
  Future<void> syncDeleteUnPaidVacationRequest() async {
    EventObject objectEventSt =
        await unPaidDeleteVacationRequest(dropUnPaidVacationRequest);
    setState(() {
  ...
  Widget build(BuildContext context) {
      ...
      child: RaisedButton(
        ...
        onPressed: () {
          if (dateFrom != null && dateTo != null) {
            pressed = true;
            syncPreviousUnPaidVacationRequest();
          } else {
            /*    Flushbar(
              title: "Failed",
    ...
    final deleteBtn = Padding(
      ...
      child: RaisedButton(
        ...
        onPressed: () {
          syncDeleteUnPaidVacationRequest();
        },
        padding: EdgeInsets.all(12),
        color: Colors.red,
    ...
    final showData = !isLoading
        ...
        : Center(
            child: DataTable(
              ...
              rows:
                  unPaidVacationRequests // Loops through dataColumnText, each iteration assigning the value to element
                      .map(
                        ((element) => DataRow(
                              cells: <DataCell>[
