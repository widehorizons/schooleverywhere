...
class _CambridgeRegisterationIDState extends State<CambridgeRegisterationID> {
  ...
  bool isLoading = false;
   String? dateValue,userSection,userAcademicYear,userStage,userGrade,userId,userType,userClass,userSemester,userChildren;
  initState() {
    super.initState();
    getLoggedInUser();
  }
  ...
  Future<void> syncCambridgeDegreeSession() async {
    ...
    if(widget.type == PARENT_TYPE) objectEvent = await getSessionCamb(userSection!, userAcademicYear!, userStage!, userGrade!, loggedParent!.regno);
    else objectEvent = await getSessionCamb(userSection!, userAcademicYear!, userStage!, userGrade!, userId!);
  ...
  Widget build(BuildContext context) {
    ...
    final goViewButton = Padding(
      ...
      child: RaisedButton(
        ...
        onPressed: () async {
          await launch(
              ApiConstants.CambridgeRegisterationID_API + "?myyears=" +
                  userAcademicYear! + "&regno=" + userId! + "&sections=" +
                  userSection! + "&stage=" + userStage! + "&garde=" + userGrade! +
                  "&class=" + userClass! + "&semister=" + userSemester! +
                  "&sess=" + sessionValue!);
        },
