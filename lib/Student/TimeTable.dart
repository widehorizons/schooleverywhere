...
class _TimeTableState extends State<TimeTable> {
  ...
  Map timeTableData = new Map();
   String? userSection,userAcademicYear,userStage,userGrade,userId,userType,userClass,userSemester,childern;
  initState() {
    super.initState();
    getLoggedInUser();
  }
  ...
  Future<void> syncTimeTablePeriod() async {
    ...
    if(widget.type == STAFF_TYPE)  objectEventDates = await getStaffTimeTable(userSection!, userAcademicYear!, userStage!, userGrade!, userClass!, userSemester!, userId!);
    else objectEventDates = await getTimeTable(userSection!, userAcademicYear!, userStage!, userGrade!, userClass!, userSemester!);
  ...
  Widget build(BuildContext context) {
    ...
    final showData = !isLoading ? Container()
        ...
        child: SingleChildScrollView(
            ...
            child: DataTable(
              columns: [
                DataColumn(label: Text(" Period/Time\n  days ",
                    style: TextStyle(
                      ...
                      cells: <DataCell>[
                        DataCell(Text(element.toString(),
                            style: TextStyle(
                                ...
                                fontWeight: FontWeight.bold,
                                fontSize: 18))),
                        for ( var names in timeTableData[element] ) //Extracting from Map element the value
    ...
    onPressed: () async {
      if(widget.type == STAFF_TYPE) {
        await launch(
            TimeTableURL.BASE_URL_TIME_TABLE_STAFF + "?myyears=" +
                userAcademicYear! + "&sections=" +
                userSection! + "&stage=" + userStage! + "&grade=" +
                userGrade! + "&staffid=" + userId! + "&semester=" +
                userSemester!);
      }else{
        await launch(
            TimeTableURL.BASE_URL_TIME_TABLE + "?myyears=" +
                userAcademicYear! + "&sections=" +
                userSection! + "&stage=" + userStage! + "&grade=" +
                userGrade! + "&class=" + userClass! + "&semester=" +
                userSemester!);
      }
    },
    color: AppTheme.appColor,
