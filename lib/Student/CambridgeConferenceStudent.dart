...
class _CambridgeConferenceStudentState extends State<CambridgeConferenceStudent> {
  ...
  Future<void> JoinConferenceStatus(String roomChannelName, String staffid, String StaffSubjectId) async{
    EventObject objectEvent = new EventObject();
    objectEvent = await JoinCambridgeConference(roomChannelName,userId!,userAcademicYear!,StaffSubjectId,userSection!,userstage!,usergrade!,userclass!,usersemester!,typeUser!);
    // print("kkkkkkk" + objectEvent.object);
    Map? data = objectEvent.object as Map?;
    if (objectEvent.success!) {
      JoinStaff = data!['data'];
                ...
                ((element) => DataRow(
                  cells: <DataCell>[
                    ...
                    DataCell(
                      ...
                      onTap: () async {
                        _joinMeeting(ApiConstants.ConferenceSchoolName+"Schooleverywhere"+element["staffid"]+element["subjectid"]+widget.sessionid);
                        JoinConferenceStatus(ApiConstants.ConferenceSchoolName+"Schooleverywhere"+element["staffid"]+element["subjectid"]+widget.sessionid,element["staffid"],element["subjectid"]);
