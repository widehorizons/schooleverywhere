...
class PreviousVacationRequest extends StatefulWidget {
  final String id;
  final String year;
  final String section;
  const PreviousVacationRequest(this.id, this.year, this.section);

  @override
  State<StatefulWidget> createState() {
    return new _PreviousVacationRequestState();
  }
}

class _PreviousVacationRequestState extends State<PreviousVacationRequest> {
  ...
  List<dynamic> dropVacationRequest = [];
  initState() {
    super.initState();
  }

  Future<void> syncPreviousVacationRequest() async {
    EventObject objectEventSt =
        await previousVacationRequest(widget.id, dateFrom.text, dateTo.text);
    if (objectEventSt.success!) {
      ...
      setState(() {
        vacationRequests = listOfColumns;
        isLoading = true;
      });
    } else {
  ...
  Future<void> syncDeleteVacationRequest() async {
    EventObject objectEventSt =
        await deleteVacationRequest(dropVacationRequest);
    setState(() {
      ...
      if (objectEventSt.success!) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (context) => PreviousVacationRequest(
      ...
      switch (_selectedIndex) {
        case 0:
          Navigator.push(context,
              new MaterialPageRoute(builder: (context) => VacationRequest()));
          break;
        case 1:
          Navigator.push(
              context,
    ...
    final showData = !isLoading
        ...
        : Center(
            child: DataTable(
              ...
              rows:
                  vacationRequests // Loops through dataColumnText, each iteration assigning the value to element
                      .map(
                        ((element) => DataRow(
                              cells: <DataCell>[
