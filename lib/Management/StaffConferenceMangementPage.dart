...
class StaffConferenceMangementPage extends StatefulWidget {
  final String type;
  final String sectionid;
  final String Id;
  final String Academicyear;

  const StaffConferenceMangementPage(
      this.type, this.sectionid, this.Id, this.Academicyear);

  @override
  State<StatefulWidget> createState() {
    return new _StaffConferenceMangementPageState();
  }
}

class _StaffConferenceMangementPageState
    extends State<StaffConferenceMangementPage> {
  String? typeOptions;
  String? typename;
  String? checkVersionCode, _projectCode;

  Future<dynamic> homePageAllPages(String linkName, String type) async {
    if (linkName == "Staff Confernce") {
      return Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => StaffConferennce()),
      );
    } else if (linkName == "Join Conference") {
      return Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => StaffConferennceJoin()),
      );
    }
  }
                    ...
                    if (!snapshot.hasData) {
                      ...
                      if ((checkVersionCode == null) ||
                        ...
                        for (int i = 0; i < y.length; i++) {
                          ...
                          if (data['page'][i].toString() == "Staff Confernce") {
